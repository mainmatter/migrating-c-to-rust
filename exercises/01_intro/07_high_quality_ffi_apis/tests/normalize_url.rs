...
/// Non-zero fill for buffers we expect to come back untouched, so that any
/// write is detectable (a zero fill would hide a stray NUL terminator).
const FILL: u8 = 0xAA;
...
fn normalize(url: &CStr, out: &mut [u8]) -> i32 {
    // SAFETY: 1. `url` is a valid NUL-terminated C string for the call.
    //         2. `out` is writable for the `out.len()` bytes we pass.
    unsafe { bm_normalize_url(url.as_ptr(), out.as_mut_ptr().cast::<c_char>(), out.len()) }
}

/// Asserts that a rejected call left the caller's buffer alone.
fn assert_untouched(out: &[u8]) {
    assert!(
        out.iter().all(|&b| b == FILL),
        "a rejected URL must leave the caller's buffer untouched"
    );
}
...
fn a_buffer_that_fits_exactly_is_accepted() {
    ...
    let mut output = vec![FILL; input.as_bytes().len() + 1];
...
fn a_buffer_one_byte_short_is_rejected() {
    ...
    let mut output = vec![FILL; input.as_bytes().len()];
...
fn a_null_url_is_reported_before_a_null_buffer() {
    // SAFETY: both a null `url` and a null `out` are documented inputs.
...
#[test]
fn an_invalid_character_is_reported_before_a_short_buffer() {
    // Validating the whole URL before writing anything means the caller hears
    // about the tab, not about the buffer -- and `out` is left alone either
    // way. An implementation that checks capacity as it goes reports the
    // buffer instead, which is why this case belongs to this exercise and not
    // to the previous one.
    let input = CString::new("a\tb").unwrap();
    let mut output = vec![FILL; 1];
